...
class ChameleonXLLMXForConditionalGeneration(ChameleonForConditionalGeneration):
    ...
    def forward(self, input_ids=None, labels=None, index_list_all=None, input_tokens=None, dpo=False, training=True, **kwargs):
        ...
        result = ChameleonForConditionalGeneration.forward(
            self, input_ids=input_ids, labels=labels, input_tokens_index=index_list_all, input_tokens=input_tokens, use_cache=False, **kwargs
        )

        if dpo:
            return result[1], labels
