...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def set_input_tokens_and_embeds(self, input_tokens_index, input_tokens):
        self.input_tokens_index = input_tokens_index
    ...
    def forward(
        ...
        if input_tokens_index is not None: 
            for i, input_token_index in enumerate(input_tokens_index):
                for j, index in enumerate(input_token_index):
                    inputs_embeds[i, index:index+1, :] = input_tokens[i, j, :].unsqueeze(0).unsqueeze(0)    
        
        if self.input_tokens_index is not None: 
            for i, input_token_index in enumerate(self.input_tokens_index):
                for j, index in enumerate(input_token_index):
                    inputs_embeds[i, index:index+1, :] = self.input_tokens[i, j, :].unsqueeze(0).unsqueeze(0)
        
        # if self.input_tokens_index is not None: 
        #     for i, input_token_index in enumerate(self.input_tokens_index):
        #         for j, index in enumerate(input_token_index):
        #             inputs_embeds[i, index:index+1, :] = self.input_tokens[i, :].unsqueeze(0).unsqueeze(0)
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def set_input_tokens_and_embeds(self, input_tokens_index, input_tokens):
        self.model.set_input_tokens_and_embeds(input_tokens_index, input_tokens)
    ...
    def forward(
        ...
        outputs = self.model(
            input_ids=input_ids,
            input_tokens_index=input_tokens_index,
